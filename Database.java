...
public class Database {
    ...
    private Database() {
        // Database really only needs to be initialized once as it is just an interface for the data
    ...
    public void addUser(String first, String last, String email, String password, String userType) {
        ...
        if (filePath != null) {
        	...
        	try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath, true))) {
        		writer.newLine();
        		writer.write(newUser.getUserID() + "," + newUser.getFirstName() + "," + newUser.getLastName() + "," + newUser.geteAddress() + "," + newUser.getPassword() + "," + newUser.getPhoneNumber() + "," + newUser.getShippingAddress());
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    }
    // public User(String firstName, String lastName, String password, String eAddress, String phoneNumber, String shippingAddress)
    public void addUser(Buyer user) {

    	try (BufferedWriter writer = new BufferedWriter(new FileWriter(BUYERS_PATH, true))) {
    		writer.newLine();
            writer.write(user.getUserID() + "," + user.getFirstName() + "," + user.getLastName() + "," + user.geteAddress() + "," + user.getPhoneNumber() + "," + user.getShippingAddress() + "," + user.getPassword());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void addUser(Seller user) {
    	...
    	try (BufferedWriter writer = new BufferedWriter(new FileWriter(SELLERS_PATH, true))) {
    		writer.newLine();
            // writer.write(user.getUserID() + "," + user.getFirstName() + "," + user.getLastName() + "," + user.geteAddress() + "," + user.getPhoneNumber() + "," + user.getShippingAddress() + "," + user.getPassword());
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void addUser(Admin user) {
    	...
    	try (BufferedWriter writer = new BufferedWriter(new FileWriter(ADMIN_PATH, true))) {
    		writer.newLine();
            // writer.write(user.getUserID() + "," + user.getFirstName() + "," + user.getLastName() + "," + user.geteAddress() + "," + user.getPhoneNumber() + "," + user.getShippingAddress() + "," + user.getPassword());
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public User getUser(int userID, String userType) {
        ...
        //String firstName, String lastName, String password, String eAddress, String phoneNumber, String shippingAddress
        if (filePath != null) {
            try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if ((Integer.parseInt(parts[0]) == userID) && (parts.length > 1)) {
                        if (userType.equals("Buyer")) {
                            return new Buyer(parts[1], parts[2], parts[4], parts[3], parts[5], parts[6], Integer.parseInt(parts[0]));
                        } else if (userType.equals("Seller")) {
                            return new Seller(parts[1], parts[2], parts[4], parts[3], parts[5], parts[6], Integer.parseInt(parts[0]));
                        } else if (userType.equals("Admin")) {
                            return new Admin(parts[1], parts[2], parts[4], parts[3], parts[5], parts[6], Integer.parseInt(parts[0]));
                        }
                    }
                }
            } catch (IOException e) {
    ...
    public int getIDfromUser(String firstName, String lastName, String password, String eAddress, String phoneNumber, String shippingAddress) {
    	return -1;
    }
    ...
    public void addBook(Book book, int sellerID) {
       try (BufferedWriter writer = new BufferedWriter(new FileWriter(BOOKS_PATH, true))) {
    		writer.newLine();
            writer.write(book.getBookID() + "," + book.getName() + "," + book.getAuthor() + "," + sellerID + "," + book.getCategory() + "," + book.getCondition() + "," + book.getBasePrice());
        } catch (IOException e) {
            e.printStackTrace();
        }
    	return;
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(BOOKS_PATH))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (Integer.parseInt(parts[0]) == bookID) {
                    ...
                    return new Book(title, author, price, getUser(sellerID, "Seller"), category, condition, bookID);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public int getLastBookID() {
        try (BufferedReader reader = new BufferedReader(new FileReader(BOOKID_PATH))) {
            ...
            BufferedWriter writer = new BufferedWriter(new FileWriter(BOOKID_PATH, false));
    ...
    public ArrayList<Book> getAllBooks() {
    	...
    	try (BufferedReader reader = new BufferedReader(new FileReader(BOOKS_PATH))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                String condition = parts[5];
                double price = Double.parseDouble(parts[6]);
                Book book = new Book(title, author, price, getUser(sellerID, "Seller"), category, condition, bookID);
                books.add(book);
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void addTransaction(Transaction transaction) {
        ...
        // transactionID, buyer id, seller id, book title, time in milliseconds, status, amount
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(BOOKS_PATH, true))) {
    		writer.newLine();
            writer.write(transaction.getTransactionID() + "," + transaction.getBuyer().getUserID() + "," + transaction.getSeller().getUserID() + "," + transaction.getBook().getBookID() + "," + transaction.getTransactionDate().getTime() + "," + transaction.getStatus() + "," + transaction.getAmount());
        } catch (IOException e) {
            e.printStackTrace();
        }
    	return;
    ...
    // public Transaction(int transactionID, Buyer buyer, User seller, Book book, Date transactionDate, String status, double amount)
    // returns transaction object with the given transactionID
    public Transaction getTransaction(int transactionID) {
        try (BufferedReader reader = new BufferedReader(new FileReader(TRANSACTIONS_PATH))) {
            String line;
            while ((line = reader.readLine()) != null) {
                String[] parts = line.split(",");
                if (Integer.parseInt(parts[0]) == transactionID) {
                    ...
                    return new Transaction(transactionID, buyer, seller, book, new Date(timeInMillis), status, amount);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public boolean ExistingUser(String email, String userType) {
        ...
        if (filePath != null) {
            try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
                ...
                while ((line = reader.readLine()) != null) {
                    String[] parts = line.split(",");
                    if (parts[3].equals(email)) {// checks if emails match
                        existing = true;
                    }
                }
            } catch (IOException e) {
    ...
    public void removeLineFromFile(String filePath, int idToRemove) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(inputFile));
            ...
            if (removed == false) {
            	System.err.println("ID " + idToRemove + " not removed from file " + filePath + " in Database");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
